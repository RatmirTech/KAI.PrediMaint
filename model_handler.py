...
class ModelHandler:
    ...
    def predict(self, input_data):
        ...
        # Создаем маппинг между "очищенными" именами (из input_data) и полными именами (из X_train)
        cleaned_to_full_mapping = {}
        for col in self.X_train.columns:
            cleaned_col = col.replace('[', '').replace(']', '').replace('<', '').replace('K', '').replace('rpm', '').replace('Nm', '').replace('min', '').strip()
            cleaned_to_full_mapping[cleaned_col] = col
        
        # Копируем данные из входного DataFrame, используя маппинг имен
        for input_col in processed_input.columns:
            if input_col in cleaned_to_full_mapping:
                full_col_name = cleaned_to_full_mapping[input_col]
                prediction_data[full_col_name] = processed_input[input_col].values
            elif input_col in self.X_train.columns:
                prediction_data[input_col] = processed_input[input_col].values
        ...
        if self.best_model_name == 'XGBoost':
            # Получаем имена признаков, которые ожидает модель XGBoost
            expected_features = self.best_model.get_booster().feature_names
            
            # Создаем DataFrame с правильными именами столбцов и порядком
            xgb_data = pd.DataFrame(index=scaled_data.index)
            
            # Заполняем данные в правильном порядке
            for expected_feature in expected_features:
                # Находим соответствующий оригинальный столбец
                original_col = None
                for col in self.X_train.columns:
                    cleaned_col = col.replace('[', '').replace(']', '').replace('<', '')
                    if cleaned_col == expected_feature:
                        original_col = col
                        break
                
                if original_col and original_col in scaled_data.columns:
                    xgb_data[expected_feature] = scaled_data[original_col]
                else:
                    raise ValueError(f"Не найден столбец для признака {expected_feature}")
