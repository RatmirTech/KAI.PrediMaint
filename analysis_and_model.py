...
def show_prediction():
    ...
    if submit_button:
        # Создаем DataFrame с введенными параметрами, используя имена столбцов,
        # которые соответствуют обученной модели
        input_data = pd.DataFrame({
            'Type': [product_type],
            'Air temperature': [air_temp],  # Убираем [K]
            'Process temperature': [process_temp],  # Убираем [K]
            'Rotational speed': [rot_speed],  # Убираем [rpm]
            'Torque': [torque],  # Убираем [Nm]
            'Tool wear': [tool_wear]  # Убираем [min]
        })
